...
const GeneratePaper = () => {
  ...
  const handleSavePaper = () => {
    ...
    const marksPerQuestion = Number(eachQMarks) || 2;
    const calculatedTotalMarks = totalMarks || String(activeQuestions.length * marksPerQuestion);
    ...
    window.alert("Paper saved successfully.");
    setShowSavePaperDialog(false);
    navigate("/dashboard", { state: { activeView: "savedpapers" } });
  };
  ...
  const sidebarContent = (showQuestionSelection || showPaperWorkspace) ? (
    ...
    <>
      ...
      <div className="mt-3 rounded-xl bg-primary text-primary-foreground p-3">
        ...
        <p className="text-xs opacity-80">Valid till 30 Dec 2026</p>
      </div>

      <nav className="mt-4 space-y-0.5">
        <button type="button" className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium text-primary bg-secondary text-left">
                        ...
                        <div className="space-y-2">
                          {activeQuestions.filter(q => selectedQuestions.includes(q.id)).map((q, idx) => (
                            <div key={q.id} className="p-2 bg-white rounded text-sm">
                              {dualMedium !== "urdu" && <p className="text-[13px] font-medium text-gray-800">{idx + 1}. {q.text}</p>}
                              {dualMedium !== "english" && (
                                <p className={`text-gray-700 mt-1 ${isUrdu(q.urdu) ? "font-urdu" : ""}`} dir={isUrdu(q.urdu) ? "rtl" : "ltr"}>{q.urdu}</p>
                              )}
                            </div>
                          ))}
                        </div>
        ...
        <div className="px-4 md:px-6 py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 shrink-0">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-primary-foreground">
              {sidebarOpen && !showQuestionSelection ? <X size={22} /> : <Menu size={22} />}
            </button>
            <div>
          ...
          <div className="text-right min-w-0">
            <p className="text-sm font-semibold text-primary-foreground truncate">YOUR SCHOOL NAME HERE</p>
            <p className="text-xs text-primary-foreground/50 truncate hidden sm:block">YOUR SCHOOL ADDRESS HERE</p>
          </div>
        </div>
      </header>
        ...
        <main className="space-y-5 min-w-0 max-w-7xl mx-auto">
          <div className={`${showPaperWorkspace ? "hidden" : "flex items-center justify-between mb-4 gap-2"}`}>
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-lg bg-card border border-border px-3 sm:px-4 py-2 text-sm font-semibold text-foreground hover:bg-secondary shrink-0"
            >
          ...
          {!selectedBoard ? (
            ...
            <div className="space-y-5">
              <h3 className="text-center text-xl sm:text-2xl font-bold text-foreground">{selectedBoard} › {selectedLevel}</h3>
              {boardSubjectsMap[selectedBoard][selectedLevel].map((group, gi) => (
                <div key={gi} className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {group.map((s) => (
                    <button
                      key={s.title}
                      type="button"
